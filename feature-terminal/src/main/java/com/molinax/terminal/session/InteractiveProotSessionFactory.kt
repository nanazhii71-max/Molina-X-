...
class InteractiveProotSessionFactory(
    ...
    fun prepareLaunchSpec(cwd: String): ProotLaunchSpec {
        ...
        val hostArgs = buildList {
            ...
            // MODIFY blueprint §8.3 (keputusan eksplisit pengguna, dimajukan dari "Fase
            // lanjutan"): bind shared storage APA ADANYA (path identik host<->guest, bukan
            // remapping ke /sdcard) supaya perilaku match dengan Termux asli. Ini murni bind
            // mount proot -- TIDAK memberi akses kalau proses app sendiri belum diberi
            // READ/WRITE_EXTERNAL_STORAGE (lihat AndroidManifest.xml + gating permission di
            // TerminalHost.kt sebelum service ini di-spawn). Path host diasumsikan selalu ada
            // (shared storage device fisik selalu ter-mount di /storage/emulated/0) -- tidak
            // dicek exists() di sini karena ini bukan direktori app-private yang bisa
            // belum-dibuat seperti tmpDir di atas.
            add("-b"); add("/storage/emulated/0:/storage/emulated/0")
            add("/bin/bash")
            add("--login")
        }
